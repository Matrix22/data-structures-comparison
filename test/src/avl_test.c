...
static uint8_t run_test(char *in_filename, char *out_filename, char *res_filename) {
    ...
    if ((fout = fopen(out_filename, "w")) == NULL) {
        printf("Query: Could not open fin for writing queries\n");
        fclose(fin);
    ...
    for (int32_t iter = 0; iter < data_size; ++iter) {
        ...
        if (query == 0) {
            ...
            temp = clock();
            error_t err = avl_insert(tree, data);
            temp = clock() - temp;
            ...
            temp = clock();
            error_t err = avl_delete(tree, data);
            temp = clock() - temp;
            ...
            temp = clock();
            uint8_t check = avl_includes(tree, data);
            temp = clock() - temp;
            ...
            temp = clock();
            error_t err = avl_modify_data(tree, old_data, new_data);
            temp = clock() - temp;
            ...
            temp = clock();
            error_t err = avl_traverse_inorder(tree, fout);
            temp = clock() - temp;
...
static void run_tests(void) {
    for (int32_t test_idx = 1; test_idx <= 30; ++test_idx) {
        ...
        snprintf(out_filename, MAX_FILE_NAME, "out/test%d.out2", test_idx);
        snprintf(res_filename, MAX_FILE_NAME, "res/avl/res%d.out", test_idx);
