...
def main():
    ...
    for i in range(1, 11, 2):
        data_set = generate_insert_test_unique_random_numbers(i, int(10 ** (i // 2 + 2)))
        generate_test_random_queries(i, int(10 ** (i // 2 + 2)), data_set)

        data_set = generate_insert_test_unique_random_numbers(i + 1, int(5 * (10 ** (i // 2 + 2))))
        generate_test_random_queries(i + 1, int(5 * (10 ** (i // 2 + 2))), data_set)

    # Inserted data : Non-unique random numbers in any order
    for i in range(1, 11, 2):
        data_set = generate_insert_test_random_numbers(i + 10, int(10 ** (i // 2 + 2)))
        generate_test_random_queries(i + 10, int(10 ** (i // 2 + 2)), data_set)

        data_set = generate_insert_test_random_numbers(i + 11, int(5 * (10 ** (i // 2 + 2))))
        generate_test_random_queries(i + 11, int(5 * (10 ** (i // 2 + 2))), data_set)

    # Inserted data : Unique ordered numbers
    for i in range(1, 11, 2):
        data_set = generate_insert_test_unique_ordered_numbers(i + 20, int(10 ** (i // 2 + 2)))
        generate_test_random_queries(i + 20, int(10 ** (i // 2 + 2)), data_set)

        data_set = generate_insert_test_unique_ordered_numbers(i + 21, int(5 * (10 ** (i // 2 + 2))))
        generate_test_random_queries(i + 21, int(5 * (10 ** (i // 2 + 2))), data_set)
